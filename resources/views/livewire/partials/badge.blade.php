        ...
        <div style="max-width: 720px;">
            <div style="background: #fff; border: 1px solid #E4E2DB; border-radius: 18px; padding: 26px;">
                ...
                <div class="wf-badge-fields" style="display: grid; grid-template-columns: 1fr 1fr; gap: 18px; margin-top: 22px;">
                    ...
                    <label><span style="font-size: 12.5px; color: #8A8880;">{{ $L['b_payType'] }}</span><select wire:model="regPayType" style="width: 100%; margin-top: 6px; padding: 11px 13px; border: 1px solid #E4E2DB; border-radius: 10px; font-size: 14px; background: #fff; cursor: pointer;">@foreach($b['payTypeOptions'] as $o)<option value="{{ $o['id'] }}" @selected($o['id']===$regPayType)>{{ $o['label'] }}</option>@endforeach</select></label>
                    {{-- app language: the worker's phone/app UI opens in this language after login --}}
                    <label style="grid-column: span 2;"><span style="font-size: 12.5px; color: #8A8880;">{{ $L['b_lang'] }}</span><select wire:model="regLang" style="width: 100%; margin-top: 6px; padding: 11px 13px; border: 1px solid #E4E2DB; border-radius: 10px; font-size: 14px; background: #fff; cursor: pointer;">@foreach($b['langOptions'] as $o)<option value="{{ $o['id'] }}" @selected($o['id']===$regLang)>{{ $o['label'] }}</option>@endforeach</select></label>
